    ...
    <header className={ pathname === `/${params.locale}/home`
      ...
      <div className="flex items-center justify-between">
        <a  key={params.locale} href={`/${params.locale}/home`} className="text-2xl hover:underline">
          ...
          <Image src="/front_images/Logo/sna_white_logo.png" alt="Logo" width={50} height={50} 
          className="logos focus:outline-none focus:ring-none focus:ring-offset-none w-full h-full"/>
        </a>
                ...
                {navbar.map((menu, i) => (
                  <div className=" pl-6" key={`${menu.label}-${i}`}>
                    <a href={menu.href} className={ 
                      ...
                      ? 'text-[17px] nav-active-color no-underline text-[#474747]'
                      : 'text-[17px] text-[#fff] no-underline' 
                    }> {menu.label} </a>
                  </div>
                ))}
                    ...
                    <DropdownMenu.Item
                      ...
                      <div className="flex items-center gap-2">
                        <div className="rounded-full flex items-center justify-center w-10 h-10 ">
                          <Image src={`${lang.icon}`} alt="Logo" width={30} height={30} className="object-cover rounded-[10px] w-full h-full"/> 
                        </div>
        ...
        <div className="flex items-center pl-2">
          <button className="md:hidden flex flex-col gap-1.5 w-6" onClick={() => setIsOpen(!isOpen)} >
        ...
        {isOpen && (
          <div className="fixed inset-0 
            ...
            <button className="absolute top-6 right-6 text-[#fff] dark:text-[#fff] text-3xl" onClick={() => setIsOpen(false)} > &times; </button>
            {navbar.map((menu, i) => (
              <a key={`${i}`}  href={menu.href} className={ 
                pathname.startsWith(menu.href)
                ? 'text-[21px] nav-active-color no-underline text-[#474747]'
                : 'text-[21px] text-[#fff]  no-underline' 
              }> {menu.label} </a>
            ))}
